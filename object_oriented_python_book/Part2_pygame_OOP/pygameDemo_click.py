    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            # mouseX, mouseY = event.pos # Could do this if we needed it
              # Check if the click was in the rect of the snake
              # If so, choose a random new location
            if snakeRect.collidepoint(event.pos):
                snakeX = random.randrange(MAX_WIDTH)
                snakeY = random.randrange(MAX_HEIGHT)
                snakeRect = pygame.Rect(snakeX, snakeY, SNAKE_WIDTH_HEIGHT, SNAKE_WIDTH_HEIGHT)
